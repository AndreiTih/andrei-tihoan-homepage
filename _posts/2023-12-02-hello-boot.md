...
I am using a Windows 10 machine, on which I've installed [Cygwin](https://www.cygwin.com/) and [nasm](https://www.nasm.us/). Nasm, which stands for the Netwide Assembler (NASM) is an assembler for the x86 CPU architecture. These are the commands I used to compile the code and start Bochs:
...
This code introduces a new directive, **org**, some new instructions **cmp**, **je**, **inc**, and what nasm calls a 'pseudo-instruction' **db**. Let's explain each one of them one by one.
...
Not to mention you'd have to manually apply the [two's complement](https://en.wikipedia.org/wiki/Two%27s_complement) method to a number to figure out its integer representation.
